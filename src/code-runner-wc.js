...
</div><div id="codetorun" class="code-knack-text code-knack-text-active" contenteditable style="/* display: none; */">${this.innerHTML}</div><div id="output_section" class="code-knack-output text-output"><div class="code-knack-output-title">Output</div><pre class="code-knack-output-content" id="result">Loading..<br></pre></div></div></pre>
      </div>
      </div>
...
async function getData(html_element) {
    ...
    if (navigator.onLine) {
	    ...
	    // loading ANSI_UP via import	
            if (ansiUpped.loaded != true) {
              try{
                ...
                console.error(`Code Runner Web Component Error: ${err.message}`)
              }
            }
            ...
            const res = await fetch("https://emkc.org/api/v2/piston/execute", {
                ...
                body: JSON.stringify({
                    ...
                    "files": [{
                        "content": ace.edit(html_element.querySelector("#codetorun")).getValue()
                    }]
                }),
        ...
        // there was a network error or API is down etc... 
            ...
            html_element.querySelector("#result").innerText = `Something went wrong! Try again.`
        }
